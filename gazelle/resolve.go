...
func importsForRule(_ *config.Config, r *rule.Rule, f *rule.File) []resolve.ImportSpec {
	...
	for _, src := range srcs {
		...
		// The exact path is what an edge target is looked up by.
		if exact := path.Join(pkg, src); exact != imp {
			specs = append(specs, resolve.ImportSpec{Lang: languageName, Imp: exact})
		}
