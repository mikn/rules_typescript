...
trap cleanup EXIT
SCRATCH_DIR="$(mktemp -d -p "${TEST_TMPDIR:-${XDG_CACHE_HOME:-$HOME/.cache}}" -t rules_ts_nextjs.XXXXXX)"
OUTPUT_BASE="$(mktemp -d -p "${TEST_TMPDIR:-${XDG_CACHE_HOME:-$HOME/.cache}}" -t rules_ts_nextjs_output.XXXXXX)"
