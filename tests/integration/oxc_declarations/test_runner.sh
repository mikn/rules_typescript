...
SCRATCH_DIR="$(mktemp -d -p "${TEST_TMPDIR:-${XDG_CACHE_HOME:-$HOME/.cache}}" -t rules_ts_oxc_decls.XXXXXX)"
OUTPUT_BASE="$(mktemp -d -p "${TEST_TMPDIR:-${XDG_CACHE_HOME:-$HOME/.cache}}" -t rules_ts_oxc_decls_output.XXXXXX)"
cleanup() {
