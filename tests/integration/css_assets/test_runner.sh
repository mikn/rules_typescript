...
SCRATCH_DIR="$(mktemp -d -p "${TEST_TMPDIR:-${XDG_CACHE_HOME:-$HOME/.cache}}" -t rules_ts_css_assets.XXXXXX)"
OUTPUT_BASE="$(mktemp -d -p "${TEST_TMPDIR:-${XDG_CACHE_HOME:-$HOME/.cache}}" -t rules_ts_css_assets_output.XXXXXX)"
