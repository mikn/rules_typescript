...
printf '%s\n' BundlerInfo DevServerInfo NodeModulesInfo NpmHoistInfo \
  NpmLinkInfo NpmPackageInfo TsConfigInfo TsInfo TsTestRunnerInfo > "${want}"
...
printf '%s\n' data declarations deps_declarations js js_maps manifest \
  npm_files npm_packages owners sources transitive_data \
  transitive_declarations transitive_es_twins transitive_js \
  transitive_js_maps tsconfig > "${want}"
sed -n '/^TsInfo = provider($/,/^)$/p' "${PROVIDERS}" \
  | sed -n 's/^        "\([a-z_]*\)": .*/\1/p' \
  | LC_ALL=C sort > "${got}"
if ! LC_ALL=C diff -u "${want}" "${got}" > "${TEST_TMPDIR}/diff"; then
  echo "TsInfo's fields differ from the pinned sixteen (-want +got):" >&2
  cat "${TEST_TMPDIR}/diff" >&2
  exit 1
fi
echo "providers.bzl defines exactly 9 providers; TsInfo has exactly 16 fields"
