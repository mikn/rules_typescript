...
printf '%s\n' data declarations js js_maps npm_packages sources \
...
sed -n '/^TsInfo = provider($/,/^)$/p' "${PROVIDERS}" \
  ...
  | LC_ALL=C sort > "${got}"
if ! LC_ALL=C diff -u "${want}" "${got}" > "${TEST_TMPDIR}/diff"; then
  echo "TsInfo's fields differ from the pinned ten (-want +got):" >&2
  cat "${TEST_TMPDIR}/diff" >&2
  exit 1
fi
echo "providers.bzl defines exactly 6 providers; TsInfo has exactly 10 fields"
