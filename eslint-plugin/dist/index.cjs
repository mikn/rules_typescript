...
var requireExplicitTypes = createRule({
  ...
  create(context) {
    function checkExportNamedDeclaration(node) {
      ...
      if (declaration.type === utils.AST_NODE_TYPES.FunctionDeclaration) {
      ...
      if (declaration.type === utils.AST_NODE_TYPES.TSDeclareFunction) {
        return;
      }
      if (declaration.type === utils.AST_NODE_TYPES.VariableDeclaration) {
        ...
        for (const declarator of declaration.declarations) {
          ...
          if (init != null && (init.type === utils.AST_NODE_TYPES.ArrowFunctionExpression || init.type === utils.AST_NODE_TYPES.FunctionExpression)) {
    ...
    function checkExportDefaultDeclaration(node) {
      ...
      if (declaration.type === utils.AST_NODE_TYPES.ArrowFunctionExpression) {
        if (!hasReturnTypeAnnotation(declaration)) {
          context.report({
            node: declaration,
      ...
      if (declaration.type === utils.AST_NODE_TYPES.Identifier) {
        return;
      }
      if (declaration.type === utils.AST_NODE_TYPES.Literal || declaration.type === utils.AST_NODE_TYPES.TemplateLiteral) {
        return;
      }
      if (declaration.type === utils.AST_NODE_TYPES.ClassDeclaration) {
        return;
      }
      if (declaration.type === utils.AST_NODE_TYPES.TSDeclareFunction || declaration.type === utils.AST_NODE_TYPES.TSInterfaceDeclaration || declaration.type === utils.AST_NODE_TYPES.TSTypeAliasDeclaration || declaration.type === utils.AST_NODE_TYPES.TSEnumDeclaration || declaration.type === utils.AST_NODE_TYPES.TSModuleDeclaration) {
        return;
      }
      context.report({
        node: declaration,
...
exports.requireExplicitTypes = requireExplicitTypes;
