...
The ruleset declares no `crate_universe` hub: `oxc-bazel`'s crates are vendored
BUILD files, declared by its own extension (COMPATIBILITY.md § rules_rust). A
hub of your own keeps the name you give it, `crates` included, and its lockfile
is checked against your rules_rust and Rust alone.
