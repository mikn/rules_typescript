...
- **Only Bazelisk required** — Node.js, Rust and the ruleset's Go tools (the binaries of its [tools release](RELEASE_PROCESS.md#tools)) are fetched hermetically, a Go SDK only under `bazel run //:gazelle`, and [pnpm too](guides/npm.md#hermetic-pnpm) if you want it. pnpm edits the lockfile and installs the checkout Gazelle lists, never builds. The first build compiles `oxc-bazel` from Rust source, which dominates the wall time.
