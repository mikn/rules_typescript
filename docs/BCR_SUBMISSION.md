...
## Prerequisites
- Bazel 9+
- Rust, through rules_rust 0.70.0 or the ruleset's 0.73.0
  (COMPATIBILITY.md § rules_rust)
- Go 1.26+
...
rules_rust builds the oxc Rust CLI from source, from the BUILD files vendored
under `oxc_cli/crates/` and the crate archives `oxc_cli/extensions.bzl`
declares (COMPATIBILITY.md § rules_rust).
