...
`oxc-bazel` is built from the BUILD files cargo-bazel renders from
`oxc_cli/Cargo.lock`, checked in under `oxc_cli/crates/`, so a Cargo edit
without a re-render builds the previous crates and nothing in the build says
so. The `crates` job of `ci.yml` runs `tools/ci/check_vendored_crates.sh`:
`tools/vendor_crates.sh` renders the tree again, at the rules_rust floor
(COMPATIBILITY.md § rules_rust), and `git status --porcelain -- oxc_cli/crates`
has to print nothing. Its message lists the files that differ; the fix is to
commit the rendering and run `bazel mod tidy` (CONTRIBUTING.md § Vendoring the
crates). The render needs the network: the floor release tarball and cargo's
registry.
