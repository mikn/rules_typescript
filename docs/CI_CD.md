...
3. **Examples Build** (`examples`)
   - One matrix leg per workspace under `examples/` — `basic`, `app`,
     ...
     Bazel invocation, `fail-fast: false`. `tanstack-app` builds
     `//... -//:app`, with the excluded target's blocker named in the workflow
...
`next_build`, and the `node_modules` fallback taken only when no JS runtime
