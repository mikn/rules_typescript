...
### A log line you will see on every run

```
gazelle: typescript: paths entry "@acme/ui/*" has 2 targets; using only "./src/ui/*" (first)
```

`compilerOptions.paths` values are arrays — TypeScript tries each in turn — and
Gazelle uses only the first. If your `tsconfig.json` was generated by
`ts_refresh_tsconfig` the discarded entry is the `./bazel-bin/…` mirror of the one
it kept, and the line is noise. If you wrote a genuine fallback chain by hand,
every entry after the first is being ignored for dep resolution.
