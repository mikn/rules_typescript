...
rules return, and the toolchains they resolve. Nine providers load from
...
`ts_bundle` returns the bundle as the one member of all four. `ts_codegen` with
...
| `use_generated_config` | `bool` | `True` selects mode 2: `ts_bundle` generates a `vite.config.mjs` and passes its path. Default `False` |
...
| Field | Type | Description |
|---|---|---|
| `pkg_dir` | `File` | The assembled `<name>_pkg/package/` directory, ready for `npm publish` |
| `tarball` | `File` | `<name>_pkg.tar`, the directory under a `package/` prefix. The rule declares and produces it on every build |
...
`ts/private/` is [volatile](../compatibility.md#volatile). The two shipped
implementations, `//vite:dev_server` and `//oj:dev_server`, return it;
`ts_dev_server(server = ...)` takes any target that does.

| Field | Type | Vite | oj | Description |
|---|---|---|---|---|
...
| `native_react_refresh` | `bool` | `False` | `True` | `True` when the server applies React Fast Refresh itself. `react_refresh = True` then fails at analysis time |
| `runtime_deps` | `depset of File` | empty | the oj binary | Everything the server needs in runfiles beyond the generated config and the npm tree |
...
tool (the `node_modules` tree builder, `ts_codegen`, `next_build`, the
bundlers): it runs on the exec platform. The two are equal under a plain host
