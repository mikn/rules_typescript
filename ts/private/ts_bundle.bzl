...
def _generate_vite_config(ctx, entry_js_file, bundle_filename, out_dir_rel, transitive_js_files, app_mode = False, user_config_file = None):
    ...
    if app_mode:
        ...
        config_content = (
            ...
            "\n" +
            (
                # When user config is present, _userRoot is declared by the import
                # block above. Use it if non-null, else fall back to htmlDir.
...
def create_bundle_action(ctx, entry_js_info, bundle_filename):
    ...
    if bundler_target and BundlerInfo in bundler_target:
        ...
        if use_generated_config:
            ...
            inputs = depset(
                [entry_js_file, config_file] + html_input_files + user_vite_config_inputs,
                transitive = [
                    all_js,
                    all_js_maps,
                    all_css,
            ...
            #   $4 = HTML file path (exec-root-relative, app mode only)
