...
`oxc-bazel` is built from source by the consumer's rules_rust, from the BUILD
files the ruleset vendors (`oxc_cli/crates/`, rendered by cargo-bazel).
cargo-bazel renders the attributes of its own rules_rust, and the consumer's
rules_rust has to load them, so the rendering is made at the floor:

- 0.70.0 is the floor: `tools/vendor_crates.sh` renders at it, and
  `//tests/integration/consumer_crates` builds a consumer on it, with Rust
  1.91.0.
- 0.73.0 is the ruleset's own `bazel_dep`; every other test builds `oxc-bazel`
  on it.
- Other versions are untested.

rules_rust takes the root module's `rust.toolchain` tags, so a consumer's Rust
compiles `oxc-bazel`; the ruleset's own, 1.98.0, applies to its own builds.
Nothing in the ruleset checks either version.
