...
- **A `config` whose `test.include` names the sources runs its tests.** The
  ...
  The attribute layer now names the compiled test files in `test.include`,
  relative to the root, so the run is the rule's `srcs` whatever the config's
  globs say; a config without `include` is unchanged.
