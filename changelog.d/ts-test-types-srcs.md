...
- **`ts_test` now takes `types_srcs`,** forwarded to the `ts_compile` it
  ...
  The attribute carries the meaning it carries on `ts_compile`. The generated
  compile's only `srcs` are the test files, so unless a dep already stages the
  declaration (a `.d.ts` in a dep's own `srcs` is passed through and sits at
  the path the entry names), `types_srcs` is what does. See
