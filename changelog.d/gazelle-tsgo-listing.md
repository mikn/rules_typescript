...
- **Gazelle lists every hand-written `tsconfig.json` through tsgo.** A run
  ...
  `compilerOptions.types` entries verbatim. A root `tsconfig.json` whose extends
  ...
  at all (a `go test` outside Bazel) says so once and goes on with no program
  listed. `-ts_verbose` also prints one line per program and, once the walk
