...
- **A directory import whose last segment is a dot-name (`./tools/.internal`)
  ...
  extension. The guard for those dropped the label before any package check
  ran, the checked-in-`BUILD`-file check included. A dot-name directory with no
  BUILD file is still dropped, now by that check.
