...
- **Gazelle writes no `types`, `types_srcs`, `path_aliases`, `path_alias_srcs`
  ...
  `--//ts:declarations`, so a BUILD file has nothing to repeat. A `types` entry
  naming a file that a `ts_codegen` at or above the tsconfig declares in `outs`
  puts that codegen in `deps`, the target that stages the declaration; a
  `paths` alias stays in the tsconfig, where the resolver reads it for `deps`
  (the first target of a fallback array, the one tsc tries first). The edit:
