...
- **`TsInfo` is the one provider a dep returns; `JsInfo` and
  ...
  `.js` no longer sits in a consumer's runfiles beside the store that holds
  it; a workspace member's hub view forwards the member's. `deps` on `ts_compile` and `ts_test` and `entry_point` on
