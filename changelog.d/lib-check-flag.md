...
- **`--//ts:lib_check` turns `skipLibCheck` off for every target in the build.**
  ...
  than an error -- the class of bug that hid the `@types/*` mapping fixed in
  this release and, before it, an unresolvable `asset_library`
  `declaration_type`. Both were found
