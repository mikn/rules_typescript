...
- **`tags = ["manual"]` on a `ts_test` now reaches every target the macro
  ...
  `ts_compile` covers `ts_test` — therefore stopped a wildcard build. `manual`
  is forwarded to all of them; every other tag still goes to the test rule
  alone, being a statement about how the test runs.
