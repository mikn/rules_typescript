...
- **A checked-in `.d.mts` or `.d.cts` is admitted wherever a `.d.ts` is.**
  `ts_compile` already classified one as a declaration and already emitted one
  for a `.mjs` / `.cjs` src, but `srcs`, `public_globals` and `types_srcs`
  refused the extension, so `import { compile } from "./compile.mjs"` beside a
  ...
  `tsc` does -- so the checked-in file is the module's only declaration, and
  the rule declares no `.d.mts` output tsgo would never write for it.
  ...
  for `./compile.mjs`, so the test importing it gets the dep edge whether or not
