...
- **A declaration file the project `tsconfig.json` names in
  ...
  through `extends`, and TypeScript resolves a relative entry against the config
  the program was invoked with, which is the generated one in `bazel-out`. So
  the entry named nothing and every global that file declares was `TS2304` in
  every directory below. Measured on the roundtrip fixture added here, with the
  ...
  declaration — it fails on the identifier, as it did before.
  ...
  the tsconfig's own directory, or a file that is not there, is logged and
  produces nothing.
