...
- **A declaration file the project `tsconfig.json` names in
  ...
  `filegroup` beside the tsconfig. The
  ...
  the tsconfig's own directory, or a file that is neither there nor named in the
  `outs` of a `ts_codegen` in the tsconfig's BUILD file, is logged and produces
  nothing. For a file such a target writes, the label is that target, not the
  filegroup.
