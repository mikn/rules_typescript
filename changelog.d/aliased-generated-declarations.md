...
- **A `path_aliases` alias now reaches the declaration a `css_library`,
  ...
  sandbox to be found, so the dep still decides the answer -- an aliased asset
  import that a wildcard ambient used to type now gets the same generated
  declaration a relative import does, `declaration_type` included.
