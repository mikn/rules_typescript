...
- **A `ts_test` runs vitest from the config's package, and `__dirname` in the
  ...
  launcher writes the config, its `config_srcs` and the generated entry into
  that directory as regular files before the run: a runfiles entry is a
  symlink, and Vite bundles a config from its realpath, so through the symlink
  `__dirname` and the walk up to `node_modules` would start in `bazel-out`.
