...
- **A relative `compilerOptions.types` entry now resolves to a generated
  ...
  so a `.d.ts` a rule generated, staged by `types_srcs`, by `srcs`, or on a dep
  edge, sat in `bazel-out` at the path the entry named and was refused at
  ...
  the label put it, and the two refusals are gone. On the
  `worker-configuration.d.ts` fixture the same target fails at analysis on the
  previous rule and type-checks on this one, with the entry written
