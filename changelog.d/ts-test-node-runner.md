...
- **`ts_test` can run node's own test runner: `runner = "node:test"`.** A test
  ...
  retries a failed relative resolution with `.ts` rewritten to `.js`, because
  oxc copies an `allowImportingTsExtensions` specifier into the `.js` it emits
  ...
  `update_snapshots`, `vitest`) is an analysis error under `"node:test"` rather
  than a silently dropped setting, as is a dep providing `CssModuleInfo`, and
  `bazel coverage` on such a target fails instead of handing Bazel an empty
  report. No `<name>.update_snapshots` target is generated for it.
