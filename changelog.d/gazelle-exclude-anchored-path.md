...
- **`# gazelle:ts_exclude` takes a path-anchored pattern, written with a leading
  ...
  not be named on its own: `vite.config.ts` in `web/BUILD.bazel` meant that
  name anywhere under `web/`, `web/**/vite.config.ts` included.
  `# gazelle:ts_exclude ./vite.config.ts` resolves the rest of the pattern
  ...
  A pattern naming a directory is read only by the rollup walk, so it drops the
  subtree under `# gazelle:ts_package_boundary tsconfig` and reaches nothing
  under the default `every-dir` mode, where that subdirectory is a package of
  its own; there `# gazelle:exclude` or a `# gazelle:ts_ignore` in the
  subdirectory drops it. The directives reference carries the table.
