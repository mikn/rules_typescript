...
- **A `@types/*` package whose entry only forwards to another package now
  ...
  instead: `TS2868: Cannot find name 'Bun'` and `TS2339: Property 'main' does
  not exist on type 'ImportMeta'`. Naming `@npm//:bun-types` in `deps` worked
  around it, since a types-only package's own entry goes in `files`.
  ...
  `/// <reference path=...>` siblings they pull in -- and writes the packages
  it names as `type_references` on the generated target. When a consumer puts
  one of those files in `files`, each name is resolved against the referencing
