...
- **Two checkouts of this repo can run the same integration test at the same
  ...
  `TEST_TMPDIR` — `<outer output base>/execroot/_main/_tmp/<per-target hash>`, so
  two targets differ in the hash and two checkouts differ in the output base in
  front of it. They are separated by construction rather than by a name nobody
  can make unique. Invoked outside `bazel test` there is no `TEST_TMPDIR`, and
  the run root falls back to `<persistent root>/runs/<hash of the
  checkout>/<test name>` — same separation, still on the same real disk as the
  caches.
- **A killed integration test no longer strands its output base under a name
  ...
  inside `TEST_TMPDIR`, and the outer Bazel clears the whole `_tmp` that it lives
  in on each `bazel test`, so an interrupted run leaves nothing that outlives
  the next invocation. On the fallback
  path the run root is stable per checkout and per test, so a killed run's
  output base is the one the next run of that test reuses and overwrites in
  place — one per test per checkout, which is the bound the old layout had,
  minus the cross-checkout collision.
