...
- **`ts_compile(types_srcs = [...])`, the declarations a relative
  ...
  It is a label list, so the file may live in another package. Unlike a
  ...
  this target (`TS1434`/`TS1005` on a broken staged declaration). What it
  declares goes unchecked, because it is a `.d.ts` under the baseline's
