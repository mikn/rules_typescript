...
- **A bare specifier the workspace never installed no longer resolves to an npm
  ...
  The refusal reads every name the lockfile mentions, not the npm inventory.
  The inventory drops packages carrying `os:`/`cpu:`/`libc:` instead of
  duplicating `platforms.bzl` in Go, and gating on it would turn that
  under-claim into a dropped dep. A tree with its own `# gazelle:ts_npm_hub`
  resolves against a second lockfile this reader never saw, so nothing there is
  refused, and a workspace with no root lockfile at all keeps every label it
  had.
