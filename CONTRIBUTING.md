...
`oxc-bazel`'s crates are vendored in remote mode: `oxc_cli/Cargo.lock` is the
resolution, `oxc_cli/crates/` the BUILD files cargo-bazel renders from it, and
`oxc_cli/extensions.bzl` turns that package into the hub
`@rules_typescript_crates` and declares the crate archives the rendering names.
A consumer's rules_rust loads those files and nothing else, so no lockfile
digest ties the ruleset to one rules_rust or Rust version.

cargo-bazel renders the attributes of its own rules_rust, so the rendering is
made at the floor, the oldest rules_rust a consumer may hold
(COMPATIBILITY.md § rules_rust). After editing `oxc_cli/Cargo.toml`:

```bash
tools/vendor_crates.sh   # cargo and the network; renders oxc_cli/crates
bazel mod tidy           # the direct crates in MODULE.bazel's use_repo
```

The script fetches the floor release, runs `//oxc_cli:crates_vendor` under
`--override_module`, and deletes two files of the rendering nothing loads:
`crates.bzl`, a wrapper declaring the hub inside the extension, and
`alias_rules.bzl`. `BAZEL` names the Bazel it runs (default `bazel`). Moving
the floor is an edit to `RULES_RUST_FLOOR` and `FLOOR_SHA256` there (the
release tarball's sha256, the BCR's `integrity` in hex) and a re-render. CI
renders the tree again and fails on any difference
(`tools/ci/check_vendored_crates.sh`, docs/CI_CD.md § Vendored Crates).

The extension loads `defs.bzl`'s `crate_repositories`, the crate archives
alone, and the hub is a `use_repo_rule` repository over the package: a hub
declared inside the extension, as `crates.bzl` does, is a repository the
extension itself generates, and `defs.bzl`'s dependency maps name it at load
time, which Bazel rejects as a lockfile entry.
