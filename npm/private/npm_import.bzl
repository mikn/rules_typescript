...
load("//npm/private:fetch_auth.bzl", _fetch_auth = "fetch_auth")
...
def _auth_for_fetch(rctx, url):
    npmrc = ""
    if rctx.attr.npmrc:
        # An attr.label is not by itself an input to the fetch: unwatched, a
        # token rotated in .npmrc would leave the fetched repository in place.
        rctx.watch(rctx.attr.npmrc)
        npmrc = rctx.read(rctx.attr.npmrc)
    return _fetch_auth(
        npmrc,
        url,
        rctx.attr.package,
...
def _move_package(rctx, package_root):
    """Packages can ship their own node_modules, so relocation needs staging."""
    entries = rctx.path(".").readdir(watch = "no")
    occupied = {entry.basename.lower(): True for entry in entries}
    for index in range(len(entries) + 1):
        staging = "_pkg" + str(index)
        if staging not in occupied:
            break
    for entry in entries:
        rctx.rename(entry.basename, staging + "/" + entry.basename)
    rctx.rename(staging, package_root)
    for entry in rctx.path(package_root).readdir(watch = "no"):
        if entry.basename in _BOUNDARY_FILES and not entry.is_dir:
            rctx.delete(entry)
...
npm_import = repository_rule(
    ...
    attrs = {
        ...
        "store": attr.string(
            mandatory = True,
            doc = "The label of this snapshot's store target in the " +
                  "lockfile's package, `@@<repo>//<package>:node_modules/" +
                  ".pnpm/<key>/node_modules/<name>`; every stanza carries " +
                  "it as NpmPackageInfo.store.",
        ),
...
def _link_target_label(member, build_text, repo):
    """The label of the target that builds a workspace member, or None.

    A member's compiling target is `@@<repo>//<member>:<basename>` in the
    lockfile's repository, and only a BUILD file there that DECLARES it earns
    the label: one naming a target Bazel cannot resolve fails analysis for
    everything that reaches the hub, while a missing target fails only what
    asks for the member.
    ...
    Args:
        ...
        repo: The lockfile's canonical repository name, "" for the main one.
    """
    ...
    if _names_a_target(text, target):
        return "@@{}//{}:{}".format(repo, member, target)
    return None

def _repo_root(rctx):
    """The root the labels this rule writes are relative to.

    The lockfile's repository, so the member paths are resolved against it
...
def _member_targets(rctx):
    ...
    for entry in rctx.attr.members.values():
        ...
        if member not in targets:
            targets[member] = _link_target_label(
                member,
                build_text,
                rctx.attr.pnpm_lock.repo_name,
            )
    return targets
...
def _member_store_label(lock, package_name):
    return "@@{}//{}:{}".format(
        lock.repo,
        lock.package,
        store_target(store_key(package_name, MEMBER_VERSION, ""), package_name),
    )

def _link_block(members, targets, manifests, lock):
    ...
    Args:
        ...
        targets: Member path -> the label of its compiling target, or None.
        manifests: Label name -> the member's package.json as text.
        lock: struct(repo, package): the lockfile's repository and package,
            where its store is declared.
    """
    lines = []
    for label_name in sorted(members):
        ...
        if not target:
            ...
            continue
        if not manifests.get(label_name):
        ...
        store = _member_store_label(lock, package_name)
...
load("@rules_typescript//npm/private:store.bzl", "virtual_store")

store_graph = json.decode({graph})

_MEMBERS = {{
{members}}}

def _files(repo):
    return Label("@{{}}//:files".format(repo))
...
def _npm_hub_impl(rctx):
    ...
    lock = struct(
        repo = rctx.attr.pnpm_lock.repo_name,
        package = rctx.attr.pnpm_lock.package,
    )

    lines = _hub_lines(rctx.attr.members)
    lines.extend(_alias_block(rctx.attr.aliases.keys(), rctx.attr.aliases))
    lines.extend(_link_block(
        rctx.attr.members,
        targets,
        rctx.attr.member_manifests,
        lock,
    ))
    rctx.file("defs.bzl", _DEFS_HEADER.format(
        lockfile = str(rctx.attr.pnpm_lock),
        graph = json.encode(rctx.attr.store_graph),
        members = _member_lines(rctx, targets),
    ))
...
npm_hub = repository_rule(
    ...
    attrs = {
        ...
        "pnpm_lock": attr.label(
            allow_single_file = True,
            doc = "The lockfile this hub was translated from: the repository " +
                  "and package its store is declared in, and the root the " +
                  "member paths are resolved against.",
        ),
        "store_graph": attr.string(
            mandatory = True,
